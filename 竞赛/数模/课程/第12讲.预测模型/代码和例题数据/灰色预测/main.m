%%  输入原始数据并做出时间序列图
clear;clc
year =[1	2	3	4	5	6	7]';  % 横坐标表示年份，写成列向量的形式（加'就表示转置）
x0 = [
161.495
108.895
80.524
...
% 画出原始数据的时间序列图
figure("Name","时间序列图"); % 因为我们的图形不止一个，因此要设置编号
...
if ERROR == 0   % 如果上述错误均没有发生时，才能执行下面的操作步骤
    ...
    %% 绘制最终的预测效果图
    figure("Name","对后面几期")  % 下面绘图中的符号m:洋红色 b:蓝色
